...
void drawJPEGFromSpiffs(String filename, int xPos, int yPos, uint8_t zoomFactor, void (*callback)()) {  
  JpegDec.decodeFile(filename.c_str());
  uint16_t *pImg;
...
//checks wether a touc point is in an intervall
boolean isInIntervall(TS_Point point, int minX, int maxX, int minY, int maxY) {
